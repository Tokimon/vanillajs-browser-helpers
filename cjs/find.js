...
var _isCollection = require('vanillajs-helpers/isCollection');
...
function check(queries, elm) {
  let string = (0, _isString2.default)(queries);
  let array = false;
  ...
  if ((0, _isArray2.default)(queries)) {
    ...
    if (!array) {
      queries = queries[0];
      string = (0, _isString2.default)(queries);
    }
  }
  ...
  if (!(0, _isDOMDocument2.default)(elm) && !(0, _isDOMNode2.default)(elm)) {
    elm = document;
  }
...
function find(queries, elm) {
  ...
  if (!multi) {
    ...
    return nodes ? !(0, _isCollection2.default)(nodes) || (0, _isDOMNode2.default)(nodes) ? [nodes] : Array.from(nodes) : [];
  }
  ...
  return Array.from(queries.reduce((set, query) => {
    ...
    if (nodes) {
      (0, _iterate2.default)(!(0, _isCollection2.default)(nodes) || (0, _isDOMNode2.default)(nodes) ? [nodes] : nodes, node => {
        set.add(node);
      });
    }
...
function findOne(queries, elm) {
  ...
  if (!multi) {
    ...
    // Return the found nodes as an Array
    return node ? !(0, _isCollection2.default)(node) || (0, _isDOMNode2.default)(node) ? node : node[0] || null : null;
  }
  ...
  while (!node && i < max) {
    ...
    if ((0, _isCollection2.default)(node) && !(0, _isDOMNode2.default)(node)) {
      node = node[0] || null;
    }
  }
